...
def store():
    ...
    inbound_file=config["path"]["std_source"]["store"]
    out_path=config["path"]["gold_source"]["store"]
    ...
    try:
        ...
        if not cnt:
            ...
            df_data=df.select(col("s_store_sk").cast("string").alias("s_store_sk"),
                              col("s_store_id").cast("string").alias("s_store_id"),
                              col("s_store_name").cast("string").alias("s_store_name"),
                              col("s_city").cast("string").alias("s_city"),
                              col("s_state").cast("string").alias("s_state"),
                              col("s_country").cast("string").alias("s_country"),
                              col("s_zip").cast("int").alias("s_zip")
                              )

            logger.info("Performing the Data Validation")
            df_filter=df_data.na.drop(subset=["s_store_sk"])
