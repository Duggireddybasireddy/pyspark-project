...
def web_site():
    ...
    inbound_file=config["path"]["std_source"]["web_site"]
    out_path=config["path"]["gold_source"]["web_site"]
    ...
    try:
        ...
        if not cnt:
            ...
            df_data=df.select(col("web_site_sk").cast("string").alias("web_site_sk"),
                              col("web_site_id").cast("string").alias("web_site_id"),
                              col("web_name").cast("string").alias("web_name"),
                              col("web_city").cast("string").alias("web_city"),
                              col("web_state").cast("string").alias("web_state"),
                              col("web_country").cast("string").alias("web_country")
                              )

            logger.info("Performing the Data Validation")
            df_filter=df_data.na.drop(subset=["web_site_sk"])
