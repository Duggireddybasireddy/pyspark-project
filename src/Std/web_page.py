...
def web_page():
    ...
    inbound_file=config["path"]["std_source"]["web_page"]
    out_path=config["path"]["gold_source"]["web_page"]
    ...
    try:
        ...
        if not cnt:
            ...
            df_data=df.select(col("wp_web_page_sk").cast("string").alias("wp_web_page_sk"),
                              col("wp_web_page_id").cast("string").alias("wp_web_page_id"),
                              col("wp_url").cast("string").alias("wp_url"),
                              col("wp_type").cast("string").alias("wp_type")
                              )

            logger.info("Performing the Data Validation")
            df_filter=df_data.na.drop(subset=["wp_web_page_sk"])
