...
from src.common.file_name import get_file_name
...
def customer_addr():
    ...
    inbound_file=config["path"]["std_source"]["customer_address"]
    out_path=config["path"]["gold_source"]["customer_address"]
    ...
    try:
        ...
        if not cnt:
            ...
            df_data=df.select(col("ca_address_sk").cast("string").alias("ca_address_sk"),
                            col("ca_country").cast("string").alias("ca_country"),
                            col("ca_state").cast("string").alias("ca_state"),
                            col("ca_zip").cast("string").alias("ca_zip")
                            )
            logger.info("Droping the data based on ca_address_sk, when ca_address_sk is null")
            df_filter = df_data.na.drop(subset=["ca_address_sk"])
