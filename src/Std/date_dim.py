...
def date_dim():
    ...
    inbound_file=config["path"]["std_source"]["date_dim"]
    out_path=config["path"]["gold_source"]["date_dim"]
    ...
    try:
        ...
        if not cnt:
            ...
            df_data=df.select(col("d_date_sk").cast("string").alias("d_date_sk"),
                              to_date(col("d_date")).alias("d_date"))

            logger.info("Filtering the d_date_sk, when null")
            df_filter = df_data.na.drop(subset=["d_date_sk"])
