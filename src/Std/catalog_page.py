...
def catalog_page():
    ...
    inbound_file=config["path"]["std_source"]["catalog_page"]
    out_path=config["path"]["gold_source"]["catalog_webpage"]
    ...
    try:
        ...
        if df.rdd.isEmpty():
            ...
            df_data=df.select(col("cp_catalog_page_sk").cast("string").alias("cp_catalog_page_sk"),
                              col("cp_catalog_page_id").cast("string").alias("cp_catalog_page_id"),
                              col("cp_department").cast("string").alias("cp_department"),
                              col("cp_catalog_number").cast("string").alias("cp_catalog_number")
                              )

            logger.info("Performing the Data Validation")
            df_filter=df_data.na.drop(subset=["cp_catalog_page_sk"])
            cnt=df.count()
