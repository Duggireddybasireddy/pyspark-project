...
def household():
    ...
    inbound_file=config["path"]["std_source"]["household_demographics"]
    out_path=config["path"]["gold_source"]["household_demographics"]
    ...
    try:
        ...
        if not cnt:
            ...
            df_data=df.select(col("hd_demo_sk").cast("string").alias("hd_demo_sk"),
                              col("hd_income_band_sk").cast("string").alias("hd_income_band_sk"))

            logger.info("Filtering the d_date_sk, when null")
            df_filter = df_data.na.drop(subset=["hd_demo_sk", "hd_income_band_sk"])
