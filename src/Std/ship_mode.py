...
def ship_mode():
    ...
    inbound_file=config["path"]["std_source"]["ship_mode"]
    out_path=config["path"]["gold_source"]["ship_mode"]
    ...
    try:
        ...
        if not cnt:
            ...
            df_data=df.select(col("sm_ship_mode_sk").cast("string").alias("sm_ship_mode_sk"),
                              col("sm_ship_mode_id").cast("string").alias("sm_ship_mode_id"),
                              col("sm_type").cast("string").alias("sm_type"),
                              col("sm_code").cast("string").alias("sm_code"),
                              col("sm_carrier").cast("string").alias("sm_carrier"),
                              col("sm_contract").cast("string").alias("sm_contract")
                              )

            logger.info("Performing the Data Validation")
            df_filter=df_data.na.drop(subset=["sm_ship_mode_sk"])
            ...
            logger.info("Filtered data is saving csv files")
